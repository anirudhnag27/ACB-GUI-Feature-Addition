...
#MySQL database used for the task is named acbtask1
...
#delete PENDING_BACKLOG_DF and DB after changing Pending Courses.xls or ACBBACKLOG.xls as per comment present above from shared version
REGISTRATION_DATA = "Registration Data Sem II 2019-20(13.02.2020).xls"
global PENDING_BACKLOG_DF
...
class MainWindow:
    ...
    def formPrereq(self):
        prereq_file = pd.read_excel(PREREQ_LIST, skiprows=1)
        or_and = ""
        for i in range(len(prereq_file['Subject'])): #all 671 rows
            course1 = prereq_file['Subject'][i].strip(
            ) + prereq_file['Catalog'][i].strip() #eg. BIOF244
...
class SecondPage:
    ...
    def __init__(self):
        ...
        for i in range(len(timetable['Course ID'])): #all entries
        ...
        ###################################################
        #to map student count:
        registered = pd.read_excel(REGISTRATION_DATA) 
        global studentCountDict
        studentCountDict = {}
        for j in range(len(registered['ID'])): #all rows of registration data
            iter_course = registered['Subject'][j].strip(
            ) + registered['Catalog'][j].strip() #eg. BIOF244
            sectionNumber = '' #eg. L12
            
            if str(registered['Lecture Section No'][j]) != 'nan':
                sectionNumber = registered['Lecture Section No'][j].strip()

            elif str(registered['Practical Section No'][j]) != 'nan':
                sectionNumber = registered['Practical Section No'][j].strip()

            elif str(registered['Tutorial Section No'][j]) != 'nan':
                sectionNumber = registered['Tutorial Section No'][j].strip()

            elif str(registered['Project Section No'][j]) != 'nan':
                sectionNumber = registered['Project Section No'][j].strip()

            elif str(registered['Thesis section'][j]) != 'nan':
                sectionNumber = registered['Thesis section'][j].strip()

            courseString = iter_course + '-' + sectionNumber

            if courseString not in studentCountDict:
                studentCountDict[courseString] = 1

            else:
                studentCountDict[courseString] += 1        
        ##end for loop in registration data

        listOfTables = list(pd.read_sql_query('show tables;',engine))
        currentTableName = "stu_op" + student_id 
        if currentTableName in listOfTables: #if table already exists
            self.getXLS()
    ...
    def getXLS(self):
        ...
        currentTableName = "stu_op" + student_id 
        oldXLS = pd.read_sql_table(currentTableName,engine)
        #oldXLS was the variable name in the version shared for the task
    ...
    def validate(self):
        if self.ui.pushButton.text() == "Validate":
            ...
            currentTableName = "stu_op" + student_id 
            #previously: op.to_excel("stu_op#" + student_id + ".xls", index = False)
            op.to_sql(
                name=currentTableName, #DB table name for this student
                con=engine,
                if_exists='replace', #old table will be replaced with new one
                index=False
            )
    ...
    def courseClick(self, item):
        ...
        for i in courseToSectionsNotSelected:
            if i.split('-')[0] == course:
                section_number = i.split('-')[1] #eg. L12
                courseString = course + '-' + section_number
                if courseString not in studentCountDict:
                    break
                f = True
                currentCount = studentCountDict[courseString] #student count in section
                outputString = courseString + ':' + str(currentCount)
                self.ui.listWidget.addItems([outputString])
        if f == False and count[course] == 0:
            self.ui.listWidget.addItems(["Course not offered this sem"])
            #will execute for courses with zero registrations. 
    ...
    def sectionClickAdd(self, item):
        ...
        section = self.ui.listWidget.currentItem().text().split(':')[0]
...
def getPendingBacklog():
    ...
    pendingDF = pd.read_excel(PENDING_COURSE_LIST, skiprows = 1)

    for i in range(len(pendingDF)):
        if pendingDF['Campus ID'][i] not in list(back_list['Campus ID']):
            pendingDF.drop(i,inplace=True)

    pendingDF.to_sql(
        name='PENDING_BACKLOG_DB', # database table name
        con=engine,
        if_exists='replace',
        index=False
    )
...
if __name__ == '__main__':
    ...
    back_list = pd.read_excel(ACB_BACKLOG_LIST)
    listOfTables = list(pd.read_sql_query('show tables;',engine))
    if 'PENDING_BACKLOG_DB' not in listOfTables: 
        getPendingBacklog() #will have to execute always?

    merged = pd.read_sql('PENDING_BACKLOG_DB',engine)
